...
export default function Home({ navigate }: HomeProps) {
  ...
  return <div className="page-enter">
    ...
    <section className="py-28 px-8 md:px-16"><div className="max-w-screen-xl mx-auto">
      <div className="flex items-baseline justify-between mb-16"><div><p className="text-xs tracking-[.2em] uppercase mb-3" style={{color:'var(--color-muted)'}}>Work</p><h2 className="text-5xl md:text-7xl" style={{fontFamily:'var(--font-display)',fontWeight:520}}>Selected Work</h2></div><button onClick={()=>navigate('work')} className="hidden md:inline-flex accent-button">All projects <span aria-hidden="true">↗</span></button></div>
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-12 img-zoom cursor-pointer" onClick={()=>navigate('project','lipco-souvenir-design')}><div className="relative h-[300px]"><img src={lipcoCover} className="w-full h-full object-cover object-center" alt="LIPCO Group souvenir design work"/><div className="absolute bottom-0 p-8 text-white"><p className="text-[10px] tracking-[.2em] uppercase mb-2">Souvenir Design · Kirbyville, Missouri · 2024</p><h3 className="text-3xl" style={{fontFamily:'var(--font-display)'}}>LIPCO Group · <em>Souvenir Product Design</em></h3></div></div></div>
      </div>
      <div className="mt-8 md:hidden"><button onClick={()=>navigate('work')} className="accent-button">All projects <span aria-hidden="true">↗</span></button></div>
    </div></section>

    <section className="py-24 px-8 md:px-16" style={{backgroundColor:'var(--color-ink)'}}><div className="max-w-screen-xl mx-auto grid md:grid-cols-12 gap-16 items-center"><div className="md:col-span-7"><h2 className="text-white text-4xl md:text-6xl leading-[1.08]" style={{fontFamily:'var(--font-display)',fontWeight:430}}>Every project begins as a story about how someone <em>moves through the world.</em></h2></div><div className="md:col-span-4 md:col-start-9"><p className="leading-relaxed mb-8 text-white/55">Andrea Polk is a multidisciplinary designer working across graphic design, photography, UX research, and experience design.</p><button onClick={()=>navigate('about')} className="accent-button">About Andrea <span aria-hidden="true">↗</span></button></div></div></section>

    <section className="py-32 px-8 md:px-16"><div className="max-w-screen-xl mx-auto grid md:grid-cols-2 gap-16 items-center"><div><p className="text-xs tracking-[.2em] uppercase mb-6" style={{color:'var(--color-muted)'}}>About</p><h2 className="text-5xl md:text-6xl mb-10 leading-[1.08]" style={{fontFamily:'var(--font-display)',fontWeight:520}}>Experience-first. <em>Always.</em></h2><p className="text-sm mb-10" style={{color:'var(--color-muted)'}}>Currently at Hochschule Rhein-Waal, Germany.</p><button onClick={()=>navigate('about')} className="accent-button">Read more <span aria-hidden="true">↗</span></button></div><div className="img-zoom"><div className="aspect-[4/3]"><img src={branson3} alt="Explore Branson" className="w-full h-full object-cover"/></div></div></div></section>
  </div>
}
